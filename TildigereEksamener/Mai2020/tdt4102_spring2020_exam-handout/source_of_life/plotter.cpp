...
Color::Color_type GraphColor::next_color()
{
  ...
  if (cur_color>=colors.size()){
    throw new string("No more colors available");
  }
...
GraphColorRandom::GraphColorRandom()
{
  ...
  for(unsigned int i = 0; i < colors.size()-1; i++){
    unsigned long j = i + (rand()%(colors.size()-1));
    Color::Color_type mdt = colors[i];
    colors[i] = colors[j];
    colors[j] = mdt;
  }
...
Point Plotter::get_last_point(const string dam, Point current_point)
{
  ...
  if (last_points.find(dam) == last_points.end()){
    ...
    Point last_point = last_points[dam];
    last_points[dam] = current_point;
    return last_point;
  }
  // END: P4
...
{
  // BEGIN: P5
  if(alarm != Dam::WaterAlarm::NoAlarm){ //Sjekker om det har gått alarm
    auto s = new Line({calc_x(timestep), y_len + 40},{calc_x(timestep), y_len + 60});
...
{
  // BEGIN: P6
  auto color = get_color(dam);
  auto y = calc_y(limit);
  for(unsigned int i = 0; i<timesteps; i+=10){
    auto x = calc_x(i);
    auto l = new Line({x,y}, {x+5,y});
    l->set_color(color);
    shapes.push_back(l);
    this->attach(*l);
  }
