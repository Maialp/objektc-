...
{
  // BEGIN: D1
  return abs(ideal_level - water_level);
  // END: D1
}
...
void Dam::restore_water_level()
{
  ...
  if(water_level<ideal_level){
    if (water_level_delta()<inflow_capacity){
      water_level = ideal_level;
    }
    else{
      water_level+=inflow_capacity;
    }
  }
  else if (water_level>=ideal_level){
    if(water_level_delta()<outflow_capacity){
      water_level = ideal_level;
    }
    else{
      water_level -= outflow_capacity;
    }
  }
  // END: D3
}
