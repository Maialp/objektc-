...
void drawMaxMin(const vector<Temps>& temps)
{
	...
	// Fester akser og tekst til vinduet
	win.attach(x);
	win.attach(y);
	win.attach(zero);
	win.attach(ten);
	...
	// Grafer som vil vise temperaturdata i vinduet
	Open_polyline max_temps;
	Open_polyline min_temps;

	{
		// Introduserer her et anonymt skop, for å holde variabelen 'i' mest mulig lokal.
		int i = 0;
