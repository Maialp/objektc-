...
void addGuess(MastermindWindow &mwin, const string code, const int size, const char startLetter, const int round)
{
	 for (int i = 0; i < size; ++i){
        mwin.vr.push_back(new Rectangle{Point{padX+(2*i*padX), (5*padY)+(2*padY*round)}, padX, padY});
				 ...
				 const int round)
{
	...
	for (int i = 0; i < correctPosition; i++){
		mwin.vr.push_back(new Circle{Point{9*padX+(2*i*radCircle), (5*padY)+(2*padY*round)}, radCircle});
        mwin.vr[mwin.vr.size()-1].set_fill_color(Color::black);
        mwin.attach(mwin.vr[mwin.vr.size()-1]);
	} 
	for (int i = correctCharacter; i < (correctCharacter+correctPosition); i++){
		mwin.vr.push_back(new Circle{Point{9*padX+(2*i*radCircle), (5*padY)+(2*padY*round)}, radCircle});
        mwin.vr[mwin.vr.size()-1].set_fill_color(Color::white);
        mwin.attach(mwin.vr[mwin.vr.size()-1]);
	}
	for (int i = correctCharacter+correctPosition; i < size; i++){
		mwin.vr.push_back(new Circle{Point{9*padX+(2*i*radCircle), (5*padY)+(2*padY*round)}, radCircle});
        mwin.vr[mwin.vr.size()-1].set_fill_color(Color::gray);
        mwin.attach(mwin.vr[mwin.vr.size()-1]);
	}  */
}
